...
int main(int argc, char* argv[]) {
  ...
  bool use_root_constants = false;
  bool debug_mode = false;
  uint32_t dispatch_block_num = 128;
  ...
  for (int i = 0; i < argc; ++i) {
    if (std::string(argv[i]) == "-h" || std::string(argv[i]) == "--help") {
      ...
      std::cout << "--use_root_constants:                    use root constants to upload "
        "uniform contents. Default behaviour is using uniform buffer."
        << std::endl;
      std::cout << "--use_uniform_buffer:                    use uniform buffer to upload "
        "uiniform contents. Default behaviour is using uniform buffer."
        << std::endl;
  ...
  std::cout << "use_root_constants: " << use_root_constants << std::endl;
  std::cout << "use_uniform_buffer: " << !use_root_constants << std::endl;
  ...
  // Construct shader with string, the shader source code looks like:
...
void CSMain(uint3 groupId : SV_GroupID, uint groupIndex : SV_GroupIndex)
{
  ...
  const size_t constant_buffer_size = align * buffer_alignment_bytes;
  const D3D12_RESOURCE_DESC cbv_upload_resource_desc =
    CD3DX12_RESOURCE_DESC::Buffer(constant_buffer_size);

  uint32_t cbv_upload_buffer_num = use_batch_mode ? dispatch_times_per_frame : 1;
  std::vector<ComPtr<ID3D12Resource>> cbv_upload_buffer(cbv_upload_buffer_num);

  for (uint32_t i = 0; i < cbv_upload_buffer_num; ++i) {
    ThrowIfFailed(device->CreateCommittedResource(
  ...
  const D3D12_HEAP_PROPERTIES cbv_resource_heap =
    CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_DEFAULT);
  const D3D12_RESOURCE_DESC cbv_resource_desc =
    CD3DX12_RESOURCE_DESC::Buffer(constant_buffer_size);

  uint32_t constant_buffer_num = use_batch_mode ? dispatch_times_per_frame : 1;
